...
- Caddy reads `./caddy/Caddyfile` and serves both `mikewyantjr.com` (plus `www.` aliases) from `./websites`.
...
## Caddy configuration
- The `caddy:Caddyfile` defines two host blocks (`mikewyantjr.com`) pointing at the respective folders under `/var/www/sites` (the host path `./websites` is mounted read-only).
...
- Use the existing scripts to manage the stack:
  ...
  - `./scripts/smoke-test.sh` now verifies both `mikewyantjr.com` over the internal Caddy endpoint and the public NPM proxy.
